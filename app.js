...
let generator = document.body.getElementsByTagName('button')[0]

generator.addEventListener('click', getValue);

function getValue(){
  let paragraphOutput=document.body.querySelector('.output')
  let paragraphValue = input.value;
  paragraphValue = parseInt(paragraphValue)
  paragraphOutput.innerHTML=''
  if (paragraphValue<10){
  for (i=0;i<paragraphValue;i++){
    let printedOutput = document.createElement('p');
    paragraphOutput.appendChild(printedOutput);
    printedOutput.innerHTML = `
    <p class='output-secondary'>${text[i]}</p>
    `
    }
  } else {
    for (i=0;i<paragraphValue;i++){
      ...
      paragraphOutput.appendChild(printedOutput);
      printedOutput.innerHTML = `
      <p class='output-secondary'>${text[i%9]}</p>
      `
      }
  }
}